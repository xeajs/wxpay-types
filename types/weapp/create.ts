/**
 * @title 退款申请
 * @subTitle 当交易发生之后一段时间内，由于买家或者卖家的原因需要退款时，卖家可以通过退款接口将支付款退还给买家，微信支付将在收到退款请求并且验证成功之后，按照退款规则将支付款按原路退到买家帐号上。注意：
 * @支持的商户 
 * @path 
 */

export type AmountReq = {
...
export type CreateReqBody = {
  ...
  notify_url?: string
  /** 【退款资金来源】 若传递此参数则使用对应的资金账户退款，否则默认使用未结算资金退款（仅对老资金流商户适用）可选取值：AVAILABLE: 仅对老资金流商户适用，指定从可用余额账户出资 */
